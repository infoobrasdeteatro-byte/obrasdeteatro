-- Contabilidad: medición de los planes sin cuota comercial.
--
-- CAPTURA DE UN CAMBIO YA APLICADO. Esta migración está aplicada en el
-- proyecto remoto y consta en su historial con la versión 20260831074722,
...
-- es exactamente lo que se ejecutó contra la base de datos. Sus comentarios
-- originales van sin acentos; se conservan tal cual.
--
-- DEPENDE DE LA ANTERIOR. Esta migración redefine la misma función que
-- 20260829184659_accounting_monthly_budget y da por hecha su versión mensual.
-- El orden entre ambas importa: los timestamps lo preservan.
--
...
-- fichero lleva el timestamp remoto exacto (20260831074722) precisamente para
-- que coincida con esa entrada: `supabase db push` la reconocerá como ya
-- aplicada y la omitirá. Aun así, a diferencia de la anterior, reaplicarla
-- sería inocuo: usa CREATE OR REPLACE y un DROP NOT NULL idempotente.
